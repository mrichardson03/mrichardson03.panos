from __future__ import absolute_import, division, print_function

__metaclass__ = type

from ansible_collections.mrichardson03.panos.plugins.modules import (
    panos_schedule_object,
)

from .common.utils import ModuleTestCase

GET_RESPONSE = {
    "@status": "success",
    "@code": "19",
    "result": {
        "@total-count": "1",
        "@count": "1",
        "entry": [
            {
                "@name": "Work-Hours",
                "@location": "vsys",
                "@vsys": "vsys1",
                "schedule-type": {
                    "recurring": {
                        "weekly": {
                            "monday": {"member": ["09:00-17:00"]},
                            "tuesday": {"member": ["09:00-17:00"]},
                            "wednesday": {"member": ["09:00-17:00"]},
                            "thursday": {"member": ["09:00-17:00"]},
                            "friday": {"member": ["09:00-17:00"]},
                        }
                    }
                },
            }
        ],
    },
}


class TestPanosScheduleObject(ModuleTestCase):
    module = panos_schedule_object

    response = GET_RESPONSE

    create_args = {
        "name": "Work-Hours",
        "type": "recurring",
        "recurrence": "weekly",
        "weekly_monday_time": "09:00-17:00",
        "weekly_tuesday_time": "09:00-17:00",
        "weekly_wednesday_time": "09:00-17:00",
        "weekly_thursday_time": "09:00-17:00",
        "weekly_friday_time": "09:00-17:00",
    }

    create_result = {
        "@name": "Work-Hours",
        "schedule-type": {
            "recurring": {
                "weekly": {
                    "monday": {"member": ["09:00-17:00"]},
                    "tuesday": {"member": ["09:00-17:00"]},
                    "wednesday": {"member": ["09:00-17:00"]},
                    "thursday": {"member": ["09:00-17:00"]},
                    "friday": {"member": ["09:00-17:00"]},
                }
            }
        },
    }

    modify_args = {
        "name": "Work-Hours",
        "type": "recurring",
        "recurrence": "weekly",
        "weekly_saturday_time": "09:00-17:00",
        "weekly_sunday_time": "09:00-17:00",
    }

    modify_result = {
        "@name": "Work-Hours",
        "schedule-type": {
            "recurring": {
                "weekly": {
                    "saturday": {"member": ["09:00-17:00"]},
                    "sunday": {"member": ["09:00-17:00"]},
                }
            }
        },
    }

    delete_args = {"name": "Work-Hours", "state": "absent"}

    def test_create(self, connection_mock):
        connection_mock.send_request.side_effect = [(404, None), (200, None)]

        result = self._run_module(self.create_args)

        assert result["changed"]
        assert result["diff"]["after"] == self.create_result

    def test_create_fail(self, connection_mock):
        connection_mock.send_request.side_effect = [(404, None), (400, None)]

        result = self._run_module_fail(self.create_args)

        assert "Error creating" in result["msg"]

    def test_create_idempotent(self, connection_mock):
        connection_mock.send_request.side_effect = [
            (200, self.response),
            (200, None),
        ]

        result = self._run_module(self.create_args)

        assert not result["changed"]
        assert result["diff"]["after"] == self.create_result

    def test_modify(self, connection_mock):
        connection_mock.send_request.side_effect = [(200, self.response), (200, None)]

        result = self._run_module(self.modify_args)

        assert result["changed"]
        assert result["diff"]["after"] == self.modify_result

    def test_modify_fail(self, connection_mock):
        connection_mock.send_request.side_effect = [(200, self.response), (400, None)]

        result = self._run_module_fail(self.modify_args)

        assert "Error editing" in result["msg"]

    def test_delete(self, connection_mock):
        connection_mock.send_request.side_effect = [(200, self.response), (200, None)]

        result = self._run_module(self.delete_args)

        assert result["changed"]
        assert result["diff"]["after"] == ""

    def test_delete_fail(self, connection_mock):
        connection_mock.send_request.side_effect = [(200, self.response), (400, None)]

        module_args = {"name": "Test-One", "state": "absent"}

        result = self._run_module_fail(module_args)

        assert "Error deleting" in result["msg"]

    def test_delete_absent(self, connection_mock):
        connection_mock.send_request.side_effect = [(404, None), (200, None)]

        module_args = {"name": "Foo", "state": "absent"}

        result = self._run_module(module_args)

        assert not result["changed"]
        assert result["diff"]["before"] == ""
        assert result["diff"]["after"] == ""

    def test_daily_recurring(self, connection_mock):
        connection_mock.send_request.side_effect = [(404, None), (200, None)]

        module_args = {
            "name": "Work-Day",
            "type": "recurring",
            "recurrence": "daily",
            "daily_time": "09:00-17:00",
        }

        result = self._run_module(module_args)

        assert result["changed"]
        assert result["diff"]["after"] == {
            "@name": "Work-Day",
            "schedule-type": {
                "recurring": {"daily": {"member": ["09:00-17:00"]}},
            },
        }

    def test_non_recurring(self, connection_mock):
        connection_mock.send_request.side_effect = [(404, None), (200, None)]

        module_args = {
            "name": "Week-of-Sept-21",
            "type": "non-recurring",
            "non_recurring_date_time": "2020/09/21@09:00-2020/09/25@17:00",
        }

        result = self._run_module(module_args)

        assert result["changed"]
        assert result["diff"]["after"] == {
            "@name": "Week-of-Sept-21",
            "schedule-type": {
                "non-recurring": {"member": ["2020/09/21@09:00-2020/09/25@17:00"]}
            },
        }
